...
class ROI(GraphicsObject):
    ...
    **Signals**
    ...
    def __init__(self, pos,
                 size=Point(1, 1), angle=0.0, handleSize=5, invertible=False, maxBounds=None,
                 snapSize=1.0, scaleSnap=False, translateSnap=False, rotateSnap=False,
                 parent=None, pen=None, movable=True, removable=False):
        ...
        # keep track of whether free handles have moved since last change signal was emitted.
    ...
    def addScaleHandle(self, pos, center, axes=None, item=None, name=None, lockAspect=False, index=None):
        ...
        **Arguments**
        ...
        pos = Point(pos)
        center = Point(center)
        info = {'name': name, 'type': 's', 'center': center,
                'pos': pos, 'item': item, 'lockAspect': lockAspect}
        if pos.x() == center.x():
            info['xoff'] = True
        if pos.y() == center.y():
            info['yoff'] = True
    ...
    def movePoint(self, handle, pos, modifiers=QtCore.Qt.KeyboardModifier(), finish=True, coords='parent'):
        ...
        # Handles with a 'center' need to know their local position relative to
        # the center point (lp0, lp1)
        if 'center' in h:
            c = h['center']
        ...
        if h['type'] == 't':
            ...
            # If a handle and its center have the same x or y value, we can't scale
            # across that axis.
        ...
        **Arguments**
        ...
        if not returnMappedCoords:
            ...
            result, coords = fn.affineSlice(
                data, shape=shape, vectors=vectors, origin=origin, axes=axes, **kwds)
    ...
    def shape(self):
        if self._shape is None:
            ...
            if s is None:
                return self.path
            self._shape = s
            # beware--this can cause the view to adjust, which would immediately
...
class PolygonROI(ROI):
    ...
    def boundingRect(self):
        r = QtCore.QRectF()
        for h in self.handles:
            # |= gives the union of the two QRectFs
    ...
    **Arguments**
    ...
    def addSegment(self, h1, h2, index=None):
        ...
        for h in seg.handles:
            ...
            # have these handles take left clicks too, so that handles cannot be added
            # on top of other handles
    ...
    **Arguments**
    ...
    def getArrayRegion(self, data, img, axes=(0, 1)):
        ...
        for i in range(len(imgPts) - 1):
            ...
            o = Point(imgPts[i])
            r = fn.affineSlice(
                data, shape=(int(d.length()),), vectors=[Point(d.norm())], origin=o, axes=axes, order=1)
            rgns.append(r)
